...
This directory is the canonical documentation entry point for Runner Monitor. The Infiltrator project family uses the same baseline document roles in every repository so readers can move between projects without relearning the structure.
...
- docs/FIRST_PARTY_NATIVE.md — specialist or historical detail retained alongside the canonical baseline.
